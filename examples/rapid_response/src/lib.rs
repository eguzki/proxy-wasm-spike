...
proxy_wasm::main! {{
    ...
    proxy_wasm::set_root_context(|_| -> Box<dyn RootContext> { Box::new(MyProxyRoot{}) });
}}
...
impl RootContext for MyProxyRoot {
    ...
    fn create_http_context(&self, context_id: u32) -> Option<Box<dyn HttpContext>> {
        Some(Box::new(MyProxy { context_id }))
    }
}
...
impl HttpContext for MyProxy {
    fn on_http_request_headers(&mut self, _: usize, _: bool) -> Action {
        ...
        let _ = self.send_http_response(
            200,
            vec![],
            Some("Hello from MyProxy! Request headers logged.".as_bytes()),
        );
